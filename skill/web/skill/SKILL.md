...
metadata: {"clawdbot":{"emoji":"💳","requires":{"bins":["python3"],"env":["AUTOPAY_PKEY"]},"primaryEnv":"AUTOPAY_PKEY"}}
---

# iAutoPay Fact API

Base URL: `https://apipaymcp.okart.fun` (both dev and production)
...
3. **Buy API Key**: Call `/v1/buy-apikey` with the signed payment in the `PAYMENT-SIGNATURE` header
